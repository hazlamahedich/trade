...
## AI Trading Debate Lab Setup

This project has been configured for the AI Trading Debate Lab application.
...
1. **Start infrastructure services:**
   ```bash
   cd trade-app
   docker compose up -d db db_test redis mailhog
   ```
...
| PostgreSQL Test | 5434 | Test database |
...
The backend provides a health check endpoint at `GET /api/health`:
...
2. Add PostgreSQL 16 and Redis services
3. Configure environment variables (see `.env.example`)
4. Deploy Dockerfile from `fastapi_backend/`
