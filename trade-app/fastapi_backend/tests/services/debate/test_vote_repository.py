...
class TestGetResultOptimized:
    ...
    @pytest.mark.p1
    @pytest.mark.asyncio
    async def test_all_three_choices_breakdown(self, repo, debate_with_session):
        ...
        for i in range(3):
            await repo.create_vote(
                ...
                debate_external_id=debate_with_session.external_id,
                choice="bull",
                voter_fingerprint=f"fp_all3_bull_{i}",
            )
        for i in range(2):
            await repo.create_vote(
                debate_id=debate_with_session.id,
                debate_external_id=debate_with_session.external_id,
                choice="bear",
                voter_fingerprint=f"fp_all3_bear_{i}",
            )
        await repo.create_vote(
            debate_id=debate_with_session.id,
            debate_external_id=debate_with_session.external_id,
            choice="undecided",
            voter_fingerprint="fp_all3_und_0",
        )
    ...
    @pytest.mark.p1
    @pytest.mark.asyncio
    async def test_debate_metadata_in_result(self, repo, debate_with_session):
    ...
    @pytest.mark.p1
    @pytest.mark.asyncio
    async def test_exact_query_count(self, repo, debate_with_session):
    ...
    @pytest.mark.p2
    @pytest.mark.asyncio
    async def test_get_result_idempotent(self, repo, debate_with_session):
        """[3-3-REPO-008] Repeated get_result calls return identical results"""
        await repo.create_vote(
            debate_id=debate_with_session.id,
            debate_external_id=debate_with_session.external_id,
            choice="bull",
            voter_fingerprint="fp_idempotent_1",
        )
        await repo.create_vote(
            debate_id=debate_with_session.id,
            debate_external_id=debate_with_session.external_id,
            choice="bear",
            voter_fingerprint="fp_idempotent_2",
        )

        result1 = await repo.get_result(debate_with_session.external_id)
        result2 = await repo.get_result(debate_with_session.external_id)
    ...
    @pytest.mark.p1
    @pytest.mark.asyncio
    async def test_get_result_completed_debate(self, repo, debate_with_session):
...
class TestSentimentResultSerialization:
    @pytest.mark.p1
    @pytest.mark.asyncio
    async def test_result_response_all_camel_case_aliases(
        self, repo, debate_with_session
    ):
    ...
    @pytest.mark.p2
    @pytest.mark.asyncio
    async def test_result_model_dump_no_alias_uses_snake_case(
        self, repo, debate_with_session
    ):
