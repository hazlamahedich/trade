...
- [x] 14.1 **Playwright smoke test** — navigate to `/debates/{known-external-id}`, verify page renders, `<script type="application/ld+json">` exists in DOM, metadata present in `<head>`, OG tags, 404, redirect, above-fold content, transcript, CTA, mobile viewport. 11 E2E tests in `tests/e2e/debate-detail-seo.spec.ts`.
...
- 2026-04-15: Party-mode implementation review (Winston, Amelia, Murat, John). Quality score revised from 95 → **90/100** (unanimous). Reason: XSS via `dangerouslySetInnerHTML` was committed before review caught it — score measures artifact quality, not process. Consensus follow-ups addressed in this sprint: (1) P0: Task 14 E2E — 11 Playwright tests in `debate-detail-seo.spec.ts` covering meta, JSON-LD, OG, 404, redirect, above-fold, transcript, CTA, mobile; (2) P1: Error boundary logging — `error.tsx` now logs error + digest + stack via `useEffect`; (3) P1: sanitizeHTML unit tests — 3 tests for `</script>` XSS escaping in JSON-LD; (4) P2: Analytics hook stubs — `features/debate/utils/analytics.ts` with `trackEvent()`, client components `DebateDetailClientActions.tsx` wired into page with `trackEvent` on CTA and back link, 4 unit tests; (5) P2: ISR `revalidate=3600` → named constant `DEBATE_DETAIL_ISR_REVALIDATE_SECONDS`. Frontend suite: 530 tests, 0 failures. All new files pass lint. Remaining backlog (P3): ISR pre-rendering spike, flakiness audit after 5 CI runs, og:image (Story 5.1).
