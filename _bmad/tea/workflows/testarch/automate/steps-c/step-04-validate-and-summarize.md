...
## 4. Sync with Beads (Automatic)

**CRITICAL: Update Beads task tracking after test generation.**

Execute the Beads integration command:

```bash
bash {project-root}/.beads-hooks/bmad-integration.sh testarch-automate "{{story_key}}"
```

If the story key is not known, find it from:
- The story file being tested (from context)
- The implementation artifacts folder

**Output:** `✓ Beads status synced: tests-generated, needs-test-review`
